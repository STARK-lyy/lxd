...
def init_port_tables():
    conn = get_db()
    cur = conn.cursor()
    cur.execute("""
    CREATE TABLE IF NOT EXISTS port_quota (
        user_id TEXT PRIMARY KEY,
        max_ports INTEGER
    )""")
    cur.execute("""
...
def get_free_port(start=20000, end=40000):
    return random.randint(start, end)


@bot.command(name="port-add")
@is_main_admin()
async def port_add(ctx, user: discord.Member, limit: int):
    conn = get_db()
    cur = conn.cursor()
    cur.execute(
        "INSERT OR REPLACE INTO port_quota (user_id, max_ports) VALUES (?, ?)",
        (str(user.id), limit)
    )
    conn.commit()
    conn.close()
    ...
    await ctx.send(embed=create_success_embed(
        "Port Access Granted",
        f"{user.mention} can now forward **{limit} ports**."
    ))
...
async def port_list(ctx):
    ...
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT * FROM port_forwards WHERE user_id = ?", (user_id,))
    rows = cur.fetchall()
    conn.close()

    if not rows:
        await ctx.send(embed=create_info_embed("No Ports", "You have no active port forwards."))
        return

    text = []
    for r in rows:
        text.append(f"`{r['container_name']}` → `{r['host_port']} → {r['vps_port']}`")

    await ctx.send(embed=create_info_embed("Your Port Forwards", "\n".join(text)))


@bot.command(name="port")
async def port_forward(ctx, vps_id: str, vps_port: int):
    user_id = str(ctx.author.id)
    vps_list = vps_data.get(user_id, [])
    vps = next((v for v in vps_list if v["container_name"] == vps_id), None)

    if not vps:
        await ctx.send(embed=create_error_embed("Invalid VPS", "This VPS is not yours."))
        return

    conn = get_db()
    cur = conn.cursor()

    cur.execute("SELECT max_ports FROM port_quota WHERE user_id = ?", (user_id,))
    quota = cur.fetchone()
    if not quota:
        await ctx.send(embed=create_error_embed("No Access", "Port forwarding not enabled for you."))
        conn.close()
        return

    cur.execute("SELECT COUNT(*) AS c FROM port_forwards WHERE user_id = ?", (user_id,))
    used = cur.fetchone()["c"]

    if used >= quota["max_ports"]:
        await ctx.send(embed=create_error_embed("Limit Reached", "Port limit exceeded."))
        conn.close()
        return
    ...
    try:
        await execute_lxc(
            f"lxc config device add {vps_id} tcp{host_port} proxy "
            f"listen=tcp:0.0.0.0:{host_port} "
            f"connect=tcp:127.0.0.1:{vps_port}"
        )
    except Exception as e:
        conn.close()
    ...
    cur.execute(
        "INSERT INTO port_forwards (user_id, container_name, vps_port, host_port, created_at) VALUES (?, ?, ?, ?, ?)",
        (user_id, vps_id, vps_port, host_port, datetime.now().isoformat())
    )
    conn.commit()
    conn.close()

    dm = create_success_embed("Port Forwarded")
    add_field(dm, "VPS", vps_id, True)
    add_field(dm, "Public Port", str(host_port), True)
    add_field(dm, "Internal Port", str(vps_port), True)
    ...
    await ctx.send(embed=create_success_embed("Success", "Port forwarded. Details sent to DM."))


@bot.command(name="port-remove")
async def port_remove(ctx, host_port: int):
    user_id = str(ctx.author.id)
    conn = get_db()
    cur = conn.cursor()

    cur.execute(
        "SELECT * FROM port_forwards WHERE user_id = ? AND host_port = ?",
        (user_id, host_port)
    )
    row = cur.fetchone()
    ...
    if not row:
        conn.close()
        await ctx.send(embed=create_error_embed("Not Found", "Port not found."))
        return

    container = row["container_name"]

    try:
        await execute_lxc(f"lxc config device remove {container} tcp{host_port}")
    except Exception as e:
        conn.close()
        await ctx.send(embed=create_error_embed("Failed", str(e)))
        return
    ...
    await ctx.send(embed=create_success_embed("Port Removed", f"Port `{host_port}` removed successfully."))
